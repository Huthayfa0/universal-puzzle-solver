...
- White circles: the loop must pass through in a straight line.
- Black circles: the loop must make a 90-degree turn.
- Numbers: the sum of the lengths of the 2 straight line segments leaving that circle.
"""
...
def _are_collinear(d1, d2):
    """True if directions d1 and d2 are opposite (same line)."""
    return d1[0] + d2[0] == 0 and d1[1] + d2[1] == 0


def _are_perpendicular(d1, d2):
    """True if directions d1 and d2 are perpendicular."""
    return d1[0] * d2[0] + d1[1] * d2[1] == 0


class ShingokiSolver(BaseSolver):
    """Solver for Shingoki puzzles.

    Input: dot grid (height x width) with table (0=empty, 1=white, 2=black)
    and numbers_table (0=no number, 1-9=sum of segment lengths).
    ...
    def __init__(self, info, show_progress=True, partial_solution_callback=None, progress_interval=10.0, partial_interval=100.0):
        ...
        self.numbers_table = info.get("numbers_table")
        if not self.numbers_table:
            self.numbers_table = [[0] * self.width for _ in range(self.height)]
        ...
        for r in range(self.height):
            for c in range(self.width):
                t = self.table[r][c]
                if t in (1, 2):
                    num = self.numbers_table[r][c] if isinstance(self.numbers_table[r][c], int) else 0
                    self.circles.append((r, c, t == 1, num))  # (r, c, is_white, number)

        self.h_edges = []  # horizontal: (r, c) -> edge between (r,c) and (r,c+1)
        self.v_edges = []  # vertical: (r, c) -> edge between (r,c) and (r+1,c)
        for r in range(self.height):
    ...
    def _segment_lengths_from_cycle(self, order, used_set):
        """For each circle, compute (len1, len2) = number of edges to next circle each way."""
        if not order:
            return {}
        n = len(order)
        circle_set = {(r, c) for r, c, _, _ in self.circles}
        result = {}
        for idx, v in enumerate(order):
            if v not in circle_set:
                continue
            # Forward: count edges from v until we hit the next circle
            seg_forward = 0
            i = (idx + 1) % n
            while i != idx:
                seg_forward += 1
                if order[i] in circle_set:
                    break
                i = (i + 1) % n
            # Backward: count edges from v until we hit the next circle
            seg_back = 0
            i = (idx - 1) % n
            while i != idx:
                seg_back += 1
                if order[i] in circle_set:
                    break
                i = (i - 1) % n
            result[v] = (seg_forward, seg_back)
        return result

    def _check_circle_constraints(self, order, used_set):
        """Check white=straight, black=turn, and number sums. Return True if all pass."""
        if not order:
            return False
        adj = {}
        for (a, b) in used_set:
            adj.setdefault(a, []).append(b)
            adj.setdefault(b, []).append(a)
        circle_set = {(r, c): (is_white, num) for r, c, is_white, num in self.circles}
        seg_lengths = self._segment_lengths_from_cycle(order, used_set)

        for (r, c, is_white, num) in self.circles:
            v = (r, c)
            ...
            if is_white and not _are_collinear(d1, d2):
                return False
            if not is_white and not _are_perpendicular(d1, d2):
                return False
            if num > 0 and v in seg_lengths:
                s1, s2 = seg_lengths[v]
                if s1 + s2 != num:
                    return False
        return True
    ...
    def _solve(self, used_set, edge_idx, call_count, backtrack_count):
        ...
        if edge_idx >= len(self.all_edges):
            # Check we have one cycle and all circles are on it with correct constraints
            order = self._build_cycle_order(used_set)
            if order is None:
                return False
            circle_verts = {(r, c) for r, c, _, _ in self.circles}
        ...
        # Option 1: don't use this edge
        if self._solve(used_set, edge_idx + 1, call_count, backtrack_count):
            return True

        # Option 2: use this edge (if no vertex would exceed degree 2)
    ...
    def solve(self):
        """Solve the Shingoki puzzle. Returns dict with horizontal_walls and vertical_walls."""
